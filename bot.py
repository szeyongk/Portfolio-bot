...
PORTFOLIO = [
dict(ticker='AVUV', name='Amer Century SCV', shares=6.5604, avg_cost=119.61, cost_currency='dict(ticker='GOOGL', name='Alphabet (GOOGL)', shares=7.885, avg_cost=367.079, cost_currency='dict(ticker='NVDA', name='NVIDIA', shares=18.4645, avg_cost=206.293, cost_currency='USD', dict(ticker='QQQ', name='Invesco QQQ', shares=6.9517, avg_cost=695.676, cost_currency='USD', dict(ticker='SCHD', name='Schwab Dividend', shares=30.1421, avg_cost=31.24, cost_currency='dict(ticker='VOO', name='Vanguard S&P 500', shares=22.9825, avg_cost=668.322, cost_currency='dict(ticker='VXUS', name='Vanguard Intl', shares=14.8491, avg_cost=84.306, cost_currency='dict(ticker='Z74.SI', name='Singtel (Moomoo)', shares=100, avg_cost=4.30, cost_currency='dict(ticker='Z74.SI', name='Singtel (IGM)', shares=100, avg_cost=3.287, cost_currency='SGD', dict(ticker='CRO-USD', name='Cronos (CRO)', shares=7724.05, avg_cost=0.5988, cost_currency='dict(ticker='ETH-USD', name='Ethereum (ETH)', shares=0.18215232, avg_cost=4720.75, cost_currency='dict(ticker='ETHW-USD', name='EthereumPoW', shares=0.18215232, avg_cost=0.38, cost_currency=']
MANUAL_POSITIONS = [
...
if 1.0 < fx_val < 2.0:
return fx_val
print('FX live-quote out of plausible range (' + str(fx_val) + '), trying daily close except Exception as e:
...
shares_display = ' · {:g} sh'.format(p['shares']) if p['shares'] is not None else lines.append('[{}] *{}* {}{}{}'.format(tag, t, pdisplay, day, shares_display))
...
lines.append('PnL: {}SGD {:,.0f} ({}{}%)'.format(sign, total_pnl, sign, round(total_pct, return '\n'.join(lines), total_val, total_pct
...
'3. Give recommendation per position. Start each line with exactly one of these words: '4. Flag 1-2 things to watch next 48 hours\n\n'
...
requests.post(url, data={'chat_id': TELEGRAM_CHAT_ID, 'caption': caption, 'parse_mode': 'files={'photo': ('chart.png', buf, 'image/png')}, timeout=30)
def send_text(text):
url = 'https://api.telegram.org/bot' + TELEGRAM_BOT_TOKEN + '/sendMessage'
chunks = [text[i:i+4000] for i in range(0, len(text), 4000)]
for chunk in chunks:
resp = requests.post(url, json={'chat_id': TELEGRAM_CHAT_ID, 'text': chunk, 'parse_mode': if not resp.ok:
